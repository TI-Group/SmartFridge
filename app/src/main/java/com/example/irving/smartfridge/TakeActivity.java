...
public class TakeActivity extends Activity {
    ...
    private void onPictureTaken(final byte[] imageBytes) {
        if (imageBytes != null) {
            ...
            File file = ImageUtil.convertByteToFile(imageBytes, TakeActivity.this.getCacheDir());
    ...
    private void onItemIdentified(final String item_name){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                final String result = ItemChangeService.takeRequest(item_name, fridge_id, userId + "");

                runOnUiThread(new Runnable(){
                    @Override
                    public void run() {
                        Toast.makeText(TakeActivity.this, item_name +"上传"+result, Toast.LENGTH_SHORT).show();
                    }
                });
